...
pub fn nrldpc_decode(
    ...
    let m_rm = n_rm - (n - m);
    assert_eq!(input.dim(), n_rm * z);

    let slen = bg.iter().filter(|&n| *n != -1).count();
    let mut treg = Array2::<i16>::zeros((slen, z));
    let mut stored = Array2::<i16>::zeros((slen, z));
    ...
    while itr < max_iter {
        ...
        for i in 0..m_rm {
            ...
            for k in 0..z {
                let (mut min1, mut min2) = (i16::MAX, i16::MAX);
                let mut pos1 = 0;
